...
class Jsonable(object):
    ...
    def _objHook(obj):
        ...
        if isinstance(obj, list):
            return [Jsonable._objHook(item) for item in obj]
        else:
            return obj

    @staticmethod
    def _typeHook(typeObj):
        return typeObj in [bool, str,unicode, int, float, Jsonable, datetime.datetime, list]
...
class Pandora(Jsonable):
    __jsonattrs__ = ['partnerId','partnerAuthToken','time_offset','userId','userAuthToken']
    def __init__(self):
        self.set_proxy(None)
    ...
    def get_stations(self, *ignore):
        ...
        self.stations = [Station(i) for i in stations]
    ...
    def get_songs(self, stationId):
        playlist = self.json_call('station.getPlaylist', {'stationToken': stationId, 'additionalAudioUrl': 'HTTP_64_AACPLUS_ADTS,HTTP_128_MP3,HTTP_192_MP3'}, https=True)
        songs = []
        for i in playlist['items']:
            if 'songName' in i: # check for ads
                songs.append(Song(self, i))
        return songs
    def __str__(self):
        return '' #I know you cant be serialized
...
class Station(Jsonable):
    __jsonattrs__ = ['id','isCreator','name','useQuickMix','idToken','isQuickMix']
    def __init__(self, d):
    ...
    def hydrate(d):
        _d = {}
        _d['stationId'] = d['id']
        _d['stationToken'] = d['idToken']
        _d['isShared'] = not d['isCreator']
        _d['isQuickMix'] = d['isQuickMix']
        _d['stationName'] = d['name']
        return Station(_d)

    def transformIfShared(self, p):
        if not self.isCreator:
            logging.info("pandora: transforming station")
            p.json_call('station.transformSharedStation', {'stationToken': self.idToken})
            self.isCreator = True

    def get_playlist(self,p ):
        logging.info("pandora: Get Playlist")
        playlist = p.json_call('station.getPlaylist', {'stationToken': self.idToken, 'additionalAudioUrl': 'HTTP_64_AACPLUS_ADTS,HTTP_128_MP3,HTTP_192_MP3'}, https=True)
        songs = []
        for i in playlist['items']:
            if 'songName' in i: # check for ads
                songs.append(Song(p, i))
        return songs
    ...
    def rename(self, p, new_name):
        if new_name != self.name:
            self.transformIfShared()
            logging.info("pandora: Renaming station")
            p.json_call('station.renameStation', {'stationToken': self.idToken, 'stationName': new_name})
            self.name = new_name

    def delete(self,p ):
        logging.info("pandora: Deleting Station")
        p.json_call('station.deleteStation', {'stationToken': self.idToken})

class Song(Jsonable):
    def __init__(self, pandora, d):
        self.album = d['albumName']
        self.artist = d['artistName']
        self.audioUrl = d['additionalAudioUrl'][pandora.audio_format]
    ...
    def rate(self, p, rating):
        if self.rating != rating:
            ...
            if rating == RATE_NONE:
                ...
                p.delete_feedback(self.station.idToken, self.feedbackId)
            else:
                self.feedbackId = self.pandora.add_feedback(self.trackToken, rating)
            self.rating = rating

    def set_tired(self, p ):
        if not self.tired:
            p.json_call('user.sleepSong', {'trackToken': self.trackToken})
            self.tired = True

    def bookmark(self,p ):
        p.json_call('bookmark.addSongBookmark', {'trackToken': self.trackToken})

    def bookmark_artist(self, p ):
        p.json_call('bookmark.addArtistBookmark', {'trackToken': self.trackToken})
